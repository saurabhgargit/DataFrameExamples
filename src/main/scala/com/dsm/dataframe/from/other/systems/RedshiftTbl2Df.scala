  ...
  def main(args: Array[String]): Unit = {
    try {
      ...
      val txnDf = sparkSession.read
        ...
        .option("query", "select merchant_id, sum(amount) from PUBLIC.TXN_FCT group by merchant_id")
        .load()

      txnDf.show()

//      sparkSession.sparkContext.hadoopConfiguration.set("fs.s3n.awsAccessKeyId", s3Config.getString("access_key_infy"))
//      sparkSession.sparkContext.hadoopConfiguration.set("fs.s3n.awsSecretAccessKey", s3Config.getString("secret_access_key_infy"))
